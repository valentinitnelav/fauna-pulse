...
| Time-lapse burst interval | `30 min` | r97 "Repeat burst every" (`timeLapseIntervalSeconds`, s/min/h input): START-TO-START burst spacing; burst = photo every step for the photo duration; interval ≤ duration ⇒ continuous. Photo duration max now 24 h (unit-aware `DurationSettingField`). r173 continuous-mode fix (owner field bug): `TimeLapsePlan` cycles now advance every photo duration in continuous mode (were pinned at 0), because the tick re-arms the capture window only on a cycle CHANGE and the window hard-stops after the photo duration — r97-r172 continuous sessions captured ONLY the first photo-duration's photos; `timelapse_capture.burst` increments per block since r173 (DATA_GUIDE note) |
...
- **Time-lapse capture (round 97, `CaptureTrigger.timelapse`).** Photos on a pure
  ...
  (DATA_GUIDE). Chip appends "· camera off" while parked.
