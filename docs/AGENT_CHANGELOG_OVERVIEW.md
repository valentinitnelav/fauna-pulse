...
| Time-lapse camera sleep | off (opt-in) | r163 (perf review E3) `timeLapseCameraSleep`: fully unbind the camera between bursts (idle gap ≥ 30 s only), rebind ~10 s before the next; see the time-lapse invariant below |
...
- **Time-lapse capture (round 97, `CaptureTrigger.timelapse`).** Photos on a pure
  ...
  prewake 10 s, wake allowance 20 s) — the screen does the r94
