...
| Time-lapse camera sleep | off (opt-in) | r163 (perf review E3) `timeLapseCameraSleep`: fully unbind the camera between bursts (idle gap ≥ 30 s only), rebind `timeLapseWakeLeadSeconds` (r164, default 5 s, 1–60 s) before the next; composes with blackout (shared `_paused` guard). See the time-lapse invariant below |
...
- **Time-lapse capture (round 97, `CaptureTrigger.timelapse`).** Photos on a pure
  ...
  prewake = `timeLapseWakeLeadSeconds` r164 default 5 s, wake allowance
