...
| Logging | `logging/session_logger.dart`, `logging/device_thermal.dart`, `logging/error_reporter.dart`, `logging/crash_store.dart`, `logging/session_log_index.dart` | Append-only JSONL writer; phone-temperature reader; problem-report builder/sender; persistent crash files; r163 streaming one-pass session.jsonl index (built off the UI isolate, feeds every summary tab) |
...
| Inference FPS cap | `15` | r195: explicit inference FPS (AI-analyzed camera frames/s), no `0 = Max`; 5–120 input, but a positive camera cap is its upper bound. Lowering the camera cap lowers inference FPS and its auto-throttle floor if needed; legacy inference `0` migrates to 15. r129: deadline scheduler keeps non-divisor caps accurate |
| Camera FPS cap | `15` | r82: caps the camera HARDWARE rate (Camera2 AE fps range), the standing sensor/ISP load the gate can't touch; `0` removes this cap and still survives reload. r195: its default now matches inference FPS |
...
- **Build/release config (r158, r194):** SDK levels are PINNED in `android/app/build.gradle`
  ...
  without both allowlisted tester models (escape hatch `-PallowMissingBundledModels`).
  Release asset copying keeps only `MDV6-yolov10-c_int8_256.tflite` and
  `MDV6-yolov10-c_float16_256.tflite`; local debug weights stay untouched.
  The `fetchBundledModels` task is removed. `scripts/fetch_bundled_models.sh` remains
  a manual historical example and never runs during a build. Root `PRIVACY_POLICY.md`
  + human `CHANGELOG.md` exist.
