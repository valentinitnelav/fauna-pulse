...
| Post-processing (r135) | `postprocess/post_detector.dart` | Batch detector over a session's saved `roi_frames/` JPEGs via the plugin's camera-free `YOLO.predict` → append-only `<session>/post_detections.jsonl` (`post_start`/`post_detection`/`post_end`); resumable (done photos skipped), `_live.jpg` companions skipped when the main photo exists; injectable predictor for tests. Job settings live in shared_preferences `analysis_*`, NOT SessionConfig. Home: "Analyze saved photos" button, ✨ row badge, row long-press = analyze that session |
