...
- **Build/release config (r158, r194, r199):** SDK levels are PINNED in
  ...
  task is removed. `scripts/fetch_bundled_models.sh` remains a manual historical
  example and never runs during a build. Root `PRIVACY_POLICY.md` + human
  `CHANGELOG.md` exist.
