...
| **Turn camera off between bursts (time-lapse only)** | `off` | Round 163. Big heat/power saver for sparse time-lapses: after each burst the camera hardware is fully turned off ("parked") and turned back on shortly before the next burst (see *Camera wake lead* below) so it is warm when the first photo is due. The **preview freezes** while the camera is off — the mode chip adds "camera off" so you know it is intentional. Only takes effect when the burst timing leaves at least 30 s between bursts (otherwise the camera simply stays on). Reliability first: if the camera ever fails to deliver frames within 20 s of a wake, it stays on for the rest of the session and the failure is logged (`camera_sleep` records; see DATA_GUIDE). Off by default because turning the camera off/on is the riskier path — validate one session before a long unattended deployment. Combines with the screen-off (moon) button: the moon saves the screen, this saves the camera — use both for unattended runs. |
| **Camera wake lead (time-lapse only)** | `5 s` | Round 164. How many seconds before each burst the parked camera is turned back on, so exposure has settled when the first photo is due (focus never re-hunts — it stays locked manual). Range 1–60 s. Raise it if the first photo of each burst looks too dark or bright; lower it to keep the camera off longer. |
