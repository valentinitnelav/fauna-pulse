...
| Setting | Default | What it does / when to change |
|---|---|---|
| **Max inference rate** | `15 FPS` | Maximum camera frames per second the AI detector may analyze. This is **inference FPS**, not the camera preview frame rate. It is a ceiling rather than a promised speed because the camera, phone, and selected model may be slower. The input is an explicit 5–120 FPS, with no `0 = Max` shortcut. When the camera cap is positive, inference FPS cannot be set above it. If the camera cap is `0` (removed), inference may be set as high as 120 FPS. |
