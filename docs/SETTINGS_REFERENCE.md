...
| **Sync companion photo (stills)** | `on` | Round 108. A still physically lands up to ~1 s after the detection that triggered it (measured ~0.76 s median on the test Xiaomi), so a fast insect can be gone from the photo. With this on, every still also saves a small live-frame crop of the trigger moment next to it as `…_live.jpg` — lower resolution, but the insect is in it. ~50–200 KB extra per still. |
