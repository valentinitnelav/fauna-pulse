...
| **Photo source (capture mode)** | `auto` | Where each saved photo comes from. **fast** = crop the small live video frame (no camera stall, but small ROI → small photo). **high-res** = take a full-resolution photo and cut the ROI out (far more pixels, but briefly stalls the camera, lands a fraction of a second late — moving insects can show motion blur or be gone — and costs heat/storage). **auto** (recommended) = per photo, use the fast crop when it already meets your target size, and pay for a high-res photo only when the ROI is too small. *Renamed in round 112: this path was called "still" before, and `still` remains its value in `session.jsonl` and saved configs (frozen wire format).* |
| **Sync companion photo (high-res)** | `on` | Round 108. A high-res photo physically lands up to ~1 s after the detection that triggered it (measured ~0.76 s median on the test Xiaomi), so a fast insect can be gone from the photo. With this on, every high-res photo also saves a small live-frame crop of the trigger moment next to it as `…_live.jpg` — lower resolution, but the insect is in it. ~50–200 KB extra per photo. |
