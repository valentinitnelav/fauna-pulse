...
| **Torch during bursts, night (time-lapse only)** | `off` | Round 180. Nocturnal monitoring: the phone's LED torch comes on shortly before each burst (see *Torch lead* below), stays on through the burst, and goes off in the break — so overnight time-lapses produce usable photos without anyone standing in the field. The mode chip adds "torch" while the LED is lit. Works together with *Turn camera off between bursts* (the camera wakes at whichever lead is longer and the torch re-lights right after the camera is back on) and with the screen-off (moon) button (the torch sits on the camera, not the screen). The very first burst of a recording — and of each scheduled window, since each window restarts its burst clock — begins immediately and skips the lead. If the active lens has no flash unit a one-time notice appears and the session continues without light. Every torch flip is logged (`torch` records; see DATA_GUIDE). Science note: light can attract or repel some taxa — treat lit windows as their own observation condition. |
| **Torch lead (time-lapse only)** | `5 s` | Round 180. How many seconds before each burst the torch comes on, so the camera's automatic exposure settles under the lit scene before the first photo. Exposure typically re-converges within 1–2 s of the light coming on (it runs in the camera hardware's own loop, even while the app samples few frames between bursts); 5 s leaves margin for slow low-light behaviour. Range 1–60 s. A lead ≥ *Time between bursts* means the torch simply never turns off. |
...
| Setting | Default | What it does / when to change |
|---|---|---|
| **Max inference rate** | `15 FPS` | Maximum camera frames per second the AI detector may analyze. This is **inference FPS**, not the camera preview frame rate. It is a ceiling rather than a promised speed because the camera, phone, and selected model may be slower. Both rate inputs have a 30 FPS maximum, with no inference `0 = Max` shortcut. When the camera cap is positive, inference FPS cannot be set above it. If the camera cap is `0` (removed), inference may still be set no higher than 30 FPS. |
...
| **Camera frame rate cap** | `15 FPS` | How many frames per second the camera *hardware* captures (round 82). Different from inference FPS: inference only decides how many delivered frames the AI analyzes, while the sensor and image processor otherwise keep running at full rate the whole session. Lowering this positive cap below Max inference rate automatically lowers inference FPS to match. `0` removes the camera cap (~30 FPS on most phones), so inference may then be set higher. Capping also delays high-res photos (~0.4 s content lag at 15 FPS vs ~0.17 s uncapped on the test phone, round 110); the `_live.jpg` companion covers the trigger moment either way. |
...
| **Focus** | manual (close-up preset, ~13 cm) | Round 164: focus is **always manual** — autofocus was removed entirely (on a mounted phone it drifts onto the background and silently changes what is sharp mid-session). Every camera-screen open locks focus for a subject ~13 cm from the lens (the middle of the typical 10–20 cm flower distance, clamped to what the lens can reach); an amber dot on the focus button reminds you to check sharpness and vanishes once you adjust the Far–Near slider. Focus then stays exactly where you set it; adjusting it mid-recording is allowed and logged (`focus_change`). Lenses that report no manual-focus range (many ultra-wides) show no control and log `fixed`, as before. Logs from older versions may say `auto` — that mode no longer exists. |
...
| **Ignore tiny tile boxes** | `0 %` (off) | Round 179: also acts as a LIVE review filter — after an analysis, moving this value re-derives the cleanup numbers (and the review screen's boxes, chips and deletion marks) instantly from the recorded results, no re-analysis needed. That makes a sensitivity analysis cheap: analyze once at 0% (every box stays recorded), then tune. It can only go as low as the filter the analysis itself ran with (an amber hint appears otherwise), and unlike the analysis-time filter it is size-only (post-merge boxes carry no tile/full-pass origin). Executed deletions audit the value in the `post_cleanup` record. Round 141, fixed in 143. Drops tile detections whose box is *narrower* than this fraction of the photo side in either direction — catching both background specks (tiny both ways) and the thin sliver boxes tiling produces at tile borders (a partial insect cut by a tile edge is thin in one direction but can be long in the other). It never removes whole-photo-pass boxes, so it can only trim what tiling added, never fall below a plain run. Careful: SAHI's whole purpose is finding small insects, and a slender insect seen side-on also has a narrow side — start low (~1–2%) and check the review boxes before trusting it. |
