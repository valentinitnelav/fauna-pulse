...
- **What the check showed about the model (not a player bug):** the arthropod detector never boxes the large bumblebee in these clips. With the 0.7 square it puts confident boxes (0.72 to 0.75) on a brown leaf patch at the lower left of the square and a second box (about 0.44) next to it, so Find visits reports pairs of overlapping "visits" (#1/#2, #10/#11, #16/#17, #21/#22); with the smaller square around the bee its largest box is 0.19 × 0.08 of the frame (the bee is about 0.3 wide), at 0.68 at most. The clips film a laptop screen playing a social-media video (icons and the laptop's logo are in the frame), so screen texture and a close-up bee are likely far from the pictures the model learned from. This is the kind of problem the Video tab is meant to expose.
