...
## Round 225 (2026-09-24): video "AI later" pass 1 (native decode, ROI, detect, resumable detections file)

- **Why:** the owner wants ROI video bursts without live AI (the phone stays cool, older phones can take part) plus imported videos from collaborators and datasets, with detection, tracking and identification run afterwards. Plan approved 2026-09-24 (owner decisions: import + AI later first; video as a time-lapse sub-option later; accuracy scored by PC Python scripts; all four extras in scope). Plan file outside git: `~/.claude/plans/pasted-content-id-a09e-what-about-pure-yeti.md`. This round is Phase 1a only; nothing is visible in the app yet.
- **Shared thermal pause:** new `logging/thermal_pause.dart` (`waitWhileWarm`, `ThermalFn`, resume 3 °C below the limit), moved out of `identification_job.dart` without behaviour change (its tests still pass). The video pass uses the same helper.
- **Native decoder (`VideoFrameSource.kt`, plugin):** `info(path)` reads duration, rotation, MP4 date (UTC, pre-1980 = unset), size, codec, PTS table (frame count, mean fps) without decoding, and refuses 10-bit/HDR files (Dolby Vision, 10-bit profiles, PQ/HLG) with "Re-export it as 8-bit H.264". `open/next/close` decode with MediaCodec in ByteBuffer mode (`COLOR_FormatYUV420Flexible`, read via `getOutputImage`), sample frames by a PTS deadline (`PtsSampler`, tolerant to jitter, restarts after gaps), convert only the ROI to RGB in an integer loop (BT.601/709 and limited/full range from the output format; default 709 for 720p and up), apply rotation while writing, average down to `maxSidePx` (default 1280), and run `YOLOInstanceManager.predict`. Frames are numbered in display order (matches CVAT). ROI square uses the live crop convention (side snapped to 32, clamped). Decoder stall guard: 5 s without output ends the clip with a plain message. `YOLOPlugin.kt` routes `videoInfo/videoOpen/videoNext/videoClose` to a single-thread executor ("yolo-video"), closed on engine detach. Only boxes cross the channel.
- **Dart wrapper:** `packages/ultralytics_yolo/lib/core/video_frame_source.dart` (`VideoInfo`, `VideoChunk`, `VideoFrameBoxes`, static `VideoFrameSource`), exported from `ultralytics_yolo.dart`.
- **App driver:** `postprocess/video_detector.dart` walks `<session>/videos/*` (mp4/mov/m4v/3gp/mkv/webm) and writes append-only `<session>/video_detections.jsonl`: `video_run_start` {settings, model_name, use_gpu, clips_total, clips_pending}, `video_clip_start` {clip, start_epoch_ms, start_time_source, duration, fps, size, rotation, mime}, `raw_detections` {frame_ms, clip, pts_us, frame, boxes `[l,t,r,b,conf,class]` normalized to the whole upright frame}, `video_clip_done` (frame counts, roi_px, class_names, decode/convert/infer ms), `video_clip_error`, `video_run_end`. Clip start: `video_clip` record in session.jsonl, else MP4 date, else file time minus duration (logged as `start_time_source`). Resumes after the last analysed `pts_us` of an unfinished clip; finished clips are skipped; failed clips are retried next run; different settings throw `VideoSettingsChanged` unless `startOver`. Thermal pause before each chunk; cancel between chunks.
- **Tests:** `test/fauna_pulse/video_detector_test.dart` (5: records and logged start time, cancel + resume, settings change needs start over, unreadable clip retried, thermal pause); Kotlin `VideoFrameSourceTest` (4: sampler 30→15 fps, jitter and gaps, every-frame + resume, MP4 date parsing). `flutter analyze` clean, `flutter test test/fauna_pulse` 573 pass, plugin tests 173 pass.
- **Device check (Xiaomi 2107113SG, debug, `integration_test/video_decode_check_test.dart`):** synthetic clips made from a still photo (H.264 1080p, HEVC 1080p, H.264 4K, portrait with 90° rotation flag, 10-bit HEVC), 4 s each, analysed at 15 fps. All pass: 60 frames per clip; 10-bit refused at info stage; frame 0 boxes vs a photo run on the same picture 4/4 or 5/5 matched, mean IoU 0.97 to 0.99; centred ROI square lands on the live crop rectangle and its boxes match a photo run on the same crop (IoU 0.98 to 0.99). Speed per analysed frame (ms, decode / convert / detect / wall): 1080p H.264 3 / 22 / 69 (includes model warm-up) / 103; HEVC 1080p 7 / 24 / 34 / 74; 4K 4 / 37 / 29 / 79; portrait 5 / 27 / 35 / 76. So about 10 to 13 analysed frames per second: a 1 min clip at 15 fps takes about 1.5 min. The first conversion loop took 180 to 290 ms per frame; rewriting it (row-local arrays, output index stepped per rotation, no per-pixel function calls) brought it to 22 to 37 ms. Samsung RF8T403A3AT not connected this round: check still owed, as is a real stock-camera clip (synthetic clips have no MP4 date, so `start_time_source` was `file_time`).
- **Safety finding (in overview build/test section):** `flutter test integration_test/... -d <serial>` uninstalls the app afterwards by default, and also when installing fails (e.g. a signature mismatch), which would delete every session on the phone. Always pass `--no-uninstall`; debug builds only (installed app is signed with the local debug keystore, verified). The check's header says so.
- **Next (1b):** "Import videos…" in the home ⋮ menu, import sheet (name, editable start time, whole frame or drawn ROI square), `video_clip` records, `video_analysis_screen.dart` exposing fps, confidence, thermal limit (per the expose-new-tunables rule).

## Round 226 (2026-09-24): upstream v0.6.15 re-audit, CPU engine now uses 2 threads by default (was 1)

- **Why:** the owner saw upstream release notes (0.6.12 "multi-threaded LiteRT CPU inference", 0.6.13 "faster Android LiteRT CPU inference") and asked to bring the fork up to date and optimise where worthwhile. Upstream main (pubspec 0.6.15) unzipped at `~/InsectDetectApp/yolo-flutter-app-main/`. Full audit in `docs/PERF_AND_ROBUSTNESS_REVIEW.md` Part F; row added to `packages/ultralytics_yolo/FAUNAPULSE_FORK.md`.
- **Finding:** the fork's "CPU threads 0 = automatic" passed no thread count, and LiteRT's own default is ONE thread (0 and 1 timed identically on both phones). Upstream now uses `availableProcessors().coerceIn(1, 4)`.
- **Measured before choosing** (new `integration_test/cpu_threads_check_test.dart`, engine benchmark, forward and reverse order): 2 threads were ~1.9x faster than 1 on every bundled detector on both phones (Xiaomi MDV6 int8 83 → 47 ms, Samsung 221 → 117 ms; BioCLIP-2 on the Xiaomi 23.4 → 9.4 s per crop). 4 threads were slower than 2 on MDV6 int8 on both phones and at most ~15% faster on the 640-px detectors (~25% on BioCLIP) for twice the busy cores; 8 were slower than 2 everywhere. Full table in Part F.
- **Change:** `LiteRtModel.automaticCpuThreads()` = `availableProcessors().coerceIn(1, 2)`; the resolved count is stored in `LiteRtModel.cpuThreads` (never 0) and printed in the load log line. Applies to every CPU engine created with 0: live sessions (MDV6, the default model, has no GPU path on either phone), the video pass, photo analysis/SAHI, and the identification embedder. The engine benchmark labels the automatic row "CPU (automatic: 2 threads)". The embedder reports its resolved count (`ImageEmbedderInfo.cpuThreads`); "Test speed" shows "(automatic: 2 threads)" and `identify_start` logs `cpu_threads_used`.
- **Also ported:** output names `output0`, `output1`, … (upstream 0.6.14) are tried between `output_$i` and `Identity*`. Not ported: `clearLetterboxPadding` (the fork already clears the whole reused bitmap). Hazard noted, not changed: upstream docs now export with `nms=None` (`ultralytics>=8.4.142`), our `export-tflite-models.py` still passes `nms=False, end2end=False`.
- **Helper texts** (plain language, reason for the default): AI tab "CPU threads" and the identification screen's "CPU threads" now say 0 = automatic = 2 and why; SETTINGS_REFERENCE rows, IDENTIFICATION.md (BioCLIP numbers), overview row updated; stale "0 = the runtime's default" comments fixed in the plugin.
- **Round 225 follow-ups (uncommitted with this round):**
  - `android/app/build.gradle`: optional `debugIdSuffix` Gradle property, so a debug build can sit next to the Play build on the Samsung (different signature) without replacing it: `ORG_GRADLE_PROJECT_debugIdSuffix=.check flutter build apk --debug -t integration_test/<file>.dart`, then `adb install -r` and `am start -n com.faunapulse.app.check/com.ultralytics.yolo.MainActivity`; results in logcat `flutter:I`. Never run `flutter test/run/install` of `com.faunapulse.app` on the Samsung.
  - `integration_test/video_decode_check_test.dart`: model override (`VIDEO_CHECK_MODEL`), and the frame-0 and ROI comparisons now check only boxes clearly above the threshold (confidence ≥ 0.35; boxes near 0.25 flip with tiny JPEG vs H.264 differences).
  - **Owner's real clips** (Xiaomi original `VID_20260924_155954.mp4` 720x1280 portrait rot 90, 30 fps, 30 s; WhatsApp copy 478x850): analysed at 15 fps, 455 frames each. Xiaomi: 63 to 65 ms per frame wall (decode 7, convert 22 to 26, detect 28 to 29); WhatsApp copy 54 ms. Sure boxes matched the photo run on the reference frame (frame 0 2/2, IoU 0.978; ROI 2/2, IoU 0.958). Samsung (`.check` build, GPU): 285 to 390 ms per frame wall, detect ~240 ms, convert 27 to 130 ms (4K); same matches (IoU 0.97/0.95).
  - **Colour and orientation check:** decoded frames dumped as PNG against ffmpeg's frames of the same clips: PSNR 46 to 50 dB, mean difference ≤ 1.5 grey levels, a left-right flipped comparison only 9 to 14 dB, so colours and rotation are right.
  - **Start-time finding:** Android writes the MP4 "creation time" when recording STOPS: the Xiaomi clip named 15:59:54 carries 16:00:25 (its length is 30 s). The WhatsApp copy carries none and its file time is the download time. Round 227 fixes the start guess (file name first, stored time minus length second).
- **Verification:** `flutter analyze` (lib, test, integration_test, plugin lib) clean; `flutter test test/fauna_pulse` 573 pass; plugin tests 173 pass; `flutter build apk --debug` builds. On device the benchmark's automatic row reads "CPU (automatic: 2 threads)" and times the same as the 2-thread row (Xiaomi BioCLIP 9.7 vs 9.4 s). Not measured: sustained heat of 2 vs 1 thread over a long live session.

## Round 227 (2026-09-24): video import and "Run AI on videos" screen (Phase 1b of the video plan)

Plan: `~/.claude/plans/pasted-content-id-a09e-what-about-pure-yeti.md`, Phase 1b. Builds on round 225's detection pass.

- **Start times** (`postprocess/video_start_time.dart`, new): `guessClipStart` picks, in order, the session log, a date and time in the file name, the stored MP4 time minus the clip length (round 226 found that Android stores the STOP time), a day-only name (WhatsApp, noon), or the file time minus the length. Each guess carries `weak` and a plain-language note. The name wins over the stored time; they are flagged as uncertain when they differ by more than 2 minutes. `layOutClips` moves a weakly timed clip that would overlap the one before it to that clip's end (`after_previous`); well-timed clips never move. `VideoDetector._clipStart` uses the same guess, so import and analysis agree.
- **Import** (`postprocess/video_import.dart`, new): `planImport` (+ optional user shift), `importVideos` moves each file into `<session>/videos/` (rename, or copy + delete across volumes), with safe unique names and a unique session folder. It writes `session.jsonl` = `start_of_session` (`source: imported_video`, `file_token`, `video` summary, no config) + one `video_clip` per clip (`SessionLogger.logVideoClip`) + `end_of_session` (`ended_normally: true`, the last clip's end).
- **Plugin:** `VideoFrameSource.thumbnail(path, maxSide = 720)` returns the first frame as an upright JPEG (MediaMetadataRetriever), used to place the square.
- **Screens:**
  - `video_import_screen.dart`: lists the picked files with each start time and how sure it is, left-out files with the reason (not a video, 10-bit), and a free-space check (largest clip + 200 MB, because the picker already holds a copy). Offers the session name, "Change…" (date + time picker, shifts every clip), and a progress bar while moving. When done: "Run AI on these videos" or "Done". The picker's cache copies are cleared after the import and when leaving.
  - `video_analysis_screen.dart`: session (newest first; clips, length, size, clips analysed), model, confidence 0.25, frames per second 15 (1 to 30), area (whole picture or a square), and under Advanced: IoU 0.7 and pause temperature 40 °C. The button says Analyze / Continue (P of N clips left) / Analyze again with these settings / All clips analyzed. Changed settings ask before replacing results. While running it shows per-clip progress weighted by clip length, ms per frame, time left, the pause note, the temperature gauge and Stop. Prefs `video_analysis_*`.
  - `VideoSquareEditor`: the first frame shown full screen with the live ROI overlay (drag, pinch) and a 32-px-step slider; it returns a side snapped to a multiple of 32 video pixels.
  - `widgets/temperature_gauge.dart`, split out of the identification screen and shared with it.
- **Deviation from the plan:** the square is placed on the analysis screen, not the import screen. It changes the boxes and can be re-run with another square, and dragging a square inside a scrolling form fights the scroll. It is kept per session (prefilled from the last run's settings), not as an app setting.
- **Home:** ⋮ "Import videos…" (file_picker, video type, multiple). "Run AI on videos" appears in the action stack only when a session with videos exists. On video sessions the gear item reads "Run AI on videos", and a session row's "analysed" badge also counts `video_detections.jsonl`.
- **Summary:** imported sessions show the capture mode "Imported videos (AI runs afterwards)", camera settings as not applicable, and "no visits yet" notes. Full video rows and visits come with 1c/1e.
- **Also:** the analysis screen releases the wake lock on leaving only while a run is active (the run's own `finally` releases it otherwise).
- **Docs:** DATA_GUIDE §3 note + new §9 (`videos/`, `video_clip`, `start_time_source` values, `video_detections.jsonl` records); SETTINGS_REFERENCE "Video analysis" section; overview row; test/README (new device check, the zero-length-timer trap).
- **Tests:** `video_start_time_test.dart` (12), `video_import_test.dart` (5), a detector test for the start guess, `video_screens_test.dart` (5: the three screens at 360 px with a 48-px navigation bar, including a real import into a temp folder and the popped path; the square slider's maximum 1056 px on a 1920 × 1080 video; `VideoAnalysisPrefs` round-trip; the summary label of an imported session).
- **Device check** (`integration_test/video_import_check_test.dart`, Xiaomi 2b2dc560, debug): all 6 readable clips (the owner's two plus four test clips) give upright thumbnails (≤ 720 px, 240 to 500 ms). Difference from the upright reference frame is 3 to 8 grey levels, against 31 to 79 for the frame turned 90/180/270. Start guesses: the Xiaomi clip 15:59:54 from its name (stored 16:00:25 = stop), the WhatsApp copy 12:00 from its date only (weak, so the import screen asks for the time). The 10-bit clip was refused. The import moved copies (cache emptied); a 1 fps run analysed 82 frames on 6 clips with 0 failures, and every `video_clip_start` took the imported start (`session_log`). The home screen was not checked on the phone (it was locked); the normal debug build was reinstalled afterwards.
- **Verification:** `flutter analyze` (lib, test, integration_test, plugin lib) clean; `flutter test test/fauna_pulse` 595 pass (1 skipped); plugin tests 173 pass; `flutter build apk --debug` builds.
- **Not yet:** tracking the boxes into visits (`post_tracks.jsonl`, 1c), exports and the PC evaluation kit (1d), summary/dashboard/identification reading video results (1e). Summary rows for the video analysis settings come with 1e, when the summary reads `video_detections.jsonl`.

## Round 228 (2026-09-24): Video plan Phase 1c, finding visits in analysed videos (offline tracking) and exports

- **What it does:** "Find visits" (new Visits section on the "Run AI on videos" screen; it also runs automatically after each finished analysis) turns the boxes in `video_detections.jsonl` into visits. It uses the live tracker (ByteTrack or C-BIoU) with the same seconds-based settings. It takes seconds and never re-runs the detector, so it can be repeated with other settings.
- **One tracker builder:** `SessionConfig.buildTracker(fps)` replaces the camera screen's private builder, so live sessions and videos build their tracker the same way. `boxInRoi` moved from `SessionRecorder` to `models/roi.dart` (shared).
- **Replay loop shared:** `replayTracker` gains `onFrame(frame, tracks, events)` and `initialFps`; `ReplayFrame` keeps `clip`, `frame`, `pts_us`. The app and the PC replay harness run the same loop.
- **`postprocess/video_tracker.dart`:**
  - Tracks only clips with `video_clip_done` (others listed as `clips_left_out`), frames deduped by `pts_us`, class names from `video_clip_done`.
  - fps per stretch from the median frame gap (fallback: `analysis_fps`).
  - One tracker carries into the next clip only when its first frame comes after the previous clip's last, within the occlusion tolerance; otherwise it resets (overlapping or wrongly dated imports). Ids stay unique via an id offset.
  - Writes `post_tracks.jsonl` (`post_track_start` with settings, tracker parameters and clip lists; live-shaped `detections` and `track_event` with `time_ms` = frame time plus `clip`, `frame`, `pts_us`; `post_track_end`) under a temporary name, renamed when complete; a failed run leaves the previous file in place.
  - `readSummary` (first and last record) for the screen; `writeResultsZip` for "Share results".
- **`postprocess/track_export.dart`:** `visits.csv` (track_id, clip, start_time, start_s, end_s, duration_s, n_frames, mean_conf, class; times from the start of the clip the visit began in) and `mot/<clip>.txt` (MOTChallenge, frames from 1, video pixels; empty file for a clip without boxes). Both replaced on each run, written atomically.
- **Screen:** Visits section below the analysis: occlusion tolerance (3 s, 0.2 to 10) and minimum visit length (0.2 s, 0 to 2), with plain help text; the tracking method is shown and chosen in camera Settings. Button Find visits / Find visits again / Finding visits…; result line; amber note when the videos were analysed again since, or when settings changed; saved-files note; "Share results" (zip via share_plus). Tracking and zipping run in a background isolate. New prefs `video_analysis_occlusion_s`, `video_analysis_min_visit_s`.
- **Docs:** DATA_GUIDE §9 (new "Visits" part: files, records, confirmed-only caveat, clip carry rule); SETTINGS_REFERENCE two rows; overview row; test/README.
...
## Round 230 (2026-09-25): video plan Phase 1d, PC kit to score visits against a hand count, and the frame-rate sweep

No app change (one comment in `track_export.dart`); no new APK needed.

- **`tool/video_eval/evaluate_visits.py`** (standard library only, Python 3.8+):
  - Hand count as a plain CSV (`hand_count_template.csv`: `clip, start_s, end_s, taxon`; seconds or `m:ss`/`h:mm:ss`; a clip row without times = watched, no visit; comma, semicolon or tab, decimal commas when not comma-separated) or a BORIS *aggregated events* export (Subject as taxon unless "No focal subject", else Behavior; `--behavior` filter; POINT events and NA stops become zero-length visits).
  - BORIS guards: an observation holding several media files is refused (BORIS times run over the whole observation), and so is a non-zero "Time offset (s)" (unclear whether start/stop include it).
  - Clip names are matched without folder, extension and case, via `video_clip.file` and `original_name` from `session.jsonl`; `post_tracks.jsonl` gives the tracked clips, `clips_left_out` and, for the app's own `visits.csv`, the tracker and fps. Both are looked for next to the visits file and one folder up. Unknown clips, untracked clips and starts after the clip's end are refused with a plain message.
  - Matching per clip: hand visit widened by `--tolerance` (0.5 s) on each side, candidate pairs = positive widened overlap, greedy one-to-one by raw overlap then widened overlap. Split = hand visit with 2+ candidate app visits; merged = app visit with 2+ candidate hand visits.
  - Console line per run; `--out` tidy scores per run and clip plus `ALL` (n_true, n_app, found, missed, extra, precision, recall, f1, count_error, split, merged, visit seconds, mean duration error over hand visits with a duration, median start error); `--pairs` one row per visit with its status. Run label = file stem; `visits_<tracker>_<fps>fps` fills tracker and fps. Globs expand `~` and sort numerically.
- **`tool/video_eval/mot_to_cvat.py`:** the app's `mot/` files use the MOTChallenge results layout (`conf,-1,-1,-1`), which TrackEval reads, but CVAT's MOT 1.1 import (Datumaro reader, checked in its source) takes column 7 as "not ignored" and column 8 as a 1-based class index into `labels.txt`, and a -1 class gives a box without a label. The script writes one zip per clip (`gt/gt.txt` with `1,<class>,1`, class from `visits.csv`, and `gt/labels.txt`). Not yet tried in CVAT itself. DATA_GUIDE §9 and the `track_export.dart` comment no longer say CVAT imports the raw file.
- **`test/fauna_pulse/video_fps_sweep_test.dart`:** `thinVideoDetections(lines, fps)` keeps the `raw_detections` a run at that rate would have picked (same rule as the native PtsSampler: take once PTS ≥ next due − interval/10, grid restarts after a long gap, per clip). With `--dart-define=SWEEP_SESSION=`, runs `VideoTracker.run` for each rate (`SWEEP_FPS`, default 15,10,5,2,1; rates above the analysed one skipped) × both trackers, with the occlusion tolerance and minimum visit length of the session's last "Find visits", and writes `fps_sweep/visits_<tracker>_<fps>fps.csv` plus a copy of `session.jsonl`. Checked end to end on a synthetic 30 fps session (3 visits): 30/15/5 fps find all 3 with both trackers; at 2 fps ByteTrack loses the fast crossing and C-BIoU keeps it; at 1 fps both find only the resting insect.
- **`docs/VIDEO_ANALYSIS.md`** (new): workflow, the app's visit rule to count by, hand counting (spreadsheet, BORIS setup and export, keyframes in CVAT with the pre-annotation, SAM 3 and click-tracking helpers that need a human check, AI-generated videos for smoke tests only), scoring and an R plot, the frame-rate sweep recipe with Bjerge et al. 2021 (0.33 fps, at least two detections per track) and Sittinger et al. 2024 (~12.5 fps at 1080p, ~3.4 fps at 4K, "jumping" ids of fast insects), datasets (HyDaT, BuzzSet/BuzzSpot, own Zenodo 15096610, SA-FARI, Lindenthal), mammals and birds (MDV6 + a BioCLIP label pack), resolution. Linked from README's doc table, DATA_GUIDE §9 and SETTINGS_REFERENCE (*Frames analyzed per second*).
- **Deferred:** `evaluate_tracks.py` (TrackEval HOTA/IDF1 on keyframes) until box annotations exist.
- **Tests:** 3 thinning unit tests (30 → 10 fps per clip, long-gap restart, same rate keeps all); Python `test_evaluate_visits.py` (12: time formats, matching, split/merged, point-event tolerance, plain CSV with pre-import names and a watched clip without visits, sweep names, BORIS export with behavior filter, multi-video observation, time offset, unknown clip, start after clip end) and `test_mot_to_cvat.py` (1).
- **Verification:** `flutter analyze` (lib, test, integration_test, plugin lib) clean; `flutter test test/fauna_pulse` 632 pass (2 skipped: the replay and sweep harnesses without their defines); `python3 -m unittest` in `tool/video_eval` 13 pass.

## Round 231 (2026-09-25): video plan Phase 2a, "Video" tab in the summary plays imported clips with the AI's boxes

The owner found that the summary's Photos tab had nothing to show for an imported video session, so there was no way to see what the AI found or whether the analysed square was well placed. Phase 2 of the video plan was rewritten with the owner (plan file unchanged in name): 2a player, 2b phone measurements during "Run AI on videos", 2c frames kept per visit + identification, 2d clip cleanup + tracking old photo sessions.

- **What it does:** for an imported video session the summary's first tab is now **Video** (live sessions keep Photos; before the start record loads, the label follows whether a `videos/` folder exists, so it does not flicker). It plays the session's clips with the boxes drawn on them: tap to pause or play, 5 s back and forward, previous and next visit (1 s before the visit starts), speed 0.5× / 1× / 2× / 4×, sound off until switched on, the scrubber with the visit spans marked under it, and a list of the clip's visits (tap to jump). A dropdown picks the clip when there are several ("· N visits", "· analysed in part", "· not analysed", "· file deleted"). Two switches: **Whole frame** (the analysed square drawn, the rest dimmed) or **What the AI saw** (zoomed onto the square, hidden when the whole frame was analysed), and **Visits** (tracked boxes `#id class conf`, the ids of visits.csv, coasted boxes faded) or **All AI boxes** (every `raw_detections` box, also those Find visits did not count; hidden before Find visits). "Change square and analyse again" opens "Run AI on videos" for the session and reloads the tab and the graphs on return. The Identify section moved into `_identifySection()` and sits under the player too.
- **`postprocess/video_box_timeline.dart`** (new, pure Dart, parsed in an isolate through a non-async static wrapper so the closure does not capture the caller's Zone, the round-175 trap): per clip, the analysed frames keyed by `pts_us/1000` (the player's position; ExoPlayer and MediaExtractor both apply the initial empty edit, 50.3 ms on the Xiaomi test clip). `rawAt(ms)` / `trackedAt(ms)` return the latest analysed frame at or before the position, held for at most 1.5 × the median frame step, so gaps and the unanalysed tail of a stopped analysis never show old boxes. Tracked boxes come from `post_tracks.jsonl` `detections` (`box_in_roi` mapped back to the whole frame via `roi_px`). Visits are stale, and not shown, when `post_track_start.detections_run_ms` differs from the analysis run's first `video_run_start.time_ms`; a clip counts as tracked only when it is in `post_track_start.clips`. The square comes from `video_clip_done.roi_px`, else the run's `settings.roi` (clip not finished); none for (nearly) the whole frame. Torn last lines are skipped; a repeated time stamp keeps the last record.
- **`widgets/video_review_player.dart`** (new): one `VideoPlayerController` at a time (`mixWithOthers`, so music keeps playing), paused when the summary's tab changes, disposed with the screen; an open counter drops a slower earlier open. The overlay is a `CustomPainter` repainting from a `ValueNotifier` position: the plugin reports every 100 ms while playing, so a `Ticker` extrapolates reported + elapsed × speed (at most 400 ms × speed ahead, forward only while playing; a report less than 250 ms behind is ignored so the boxes do not step back). The zoomed view moves and scales the same video texture inside a `ClipRect` (no second decode) and draws boxes in view coordinates, so line widths and labels stay readable. The player is at most 55 % of the screen height (portrait clips), and the clip names end with an ellipsis.
- **New dependency `video_player: ^2.14.0`** (Flutter team, BSD-3; ExoPlayer/media3, Apache-2.0, hardware decoding; both covered by the About dialog's LicensePage like the other Android libraries); dev dependencies `video_player_platform_interface: ^6.9.0` and `wakelock_plus_platform_interface: ^1.5.1` for a fake player and a fake wakelock in widget tests. **APK size:** arm64 release 94.3 MB (r224 release) → 95.2 MB (+0.86 MB, of which Dart code from rounds 225 to 231 +0.22 MB compressed). The dex grew 1.04 MB uncompressed; `apkanalyzer` with the R8 mapping puts media3 at 0.90 MB and the plugin at 0.03 MB of it, so the player costs about 0.55 MB of download. (The size build overwrote the r224 arm64 APK in `build/app/outputs/flutter-apk/`, which carries the same version 0.8.0-alpha.1+13; the original was copied aside first and put back.)
- **Screen stays on while a clip plays:** video_player's Android side ignores `setPreventsDisplaySleepDuringVideoPlayback` (the platform interface default is a no-op and `video_player_android` 2.12.2 does not override it), so the screen would go dark at the phone's timeout while watching. The tab holds a `WakelockPlus` lock while the player reports playing and releases it on pause, clip change, tab change (via the pause) and dispose; errors from the plugin are only logged.
- **No lower-resolution option** (owner request discussed in the plan): the player decodes in hardware like the gallery; a smaller copy would need a transcode (minutes per clip, heat, storage), and the zoomed view needs full resolution. Revisit only if the device check shows stutter (it did not: the stutter it found was the Flutter side, fixed below).
- **Smooth 4× playback (found in the device check):** the play/pause row listened to the player itself, so its six buttons were built again on every position report (ten a second); at 4× that made 21 to 44 of about 280 frames miss 16.7 ms (debug build, build p90 10.7 to 14.7 ms). The row now listens to a `ValueNotifier<bool>` that changes only when playing starts or stops: 2 to 4 of 300 frames over 16.7 ms, build p90 4.1 to 4.6 ms, same clips and build. The box overlay and the time label also got their own `RepaintBoundary`, so the per-frame repaint stays in their layers (this alone did not change the numbers).
- **Docs:** DATA_GUIDE §9 "The Video tab: watching the boxes".
- **Device check (Xiaomi 2b2dc560, debug build, `integration_test/video_review_check_test.dart`, both test clips analysed at 10 frames per second with the owner's `arthropod_yolov11_float16.tflite`, 608 frames in 12 s on the GPU; the bundled yolo26 took 81 s):** paused boxes sit where the logged coordinates say, checked against ffmpeg renders of the same frames on the PC with the same boxes drawn, on the WhatsApp clip (478×850) and the portrait original (720×1280 stored with 90° rotation, plays upright); the original's first frame at 50.3 ms matches the timeline. "What the AI saw" shows exactly the analysed square. 4× plays 0 to 18.8 to 19.1 s of video in 5 s (see "Smooth 4× playback"). 5 minutes at 1× (the clip played 286 of the 300 s, restarted at its end): battery 31.0 °C → 32.0 °C, thermal status none, on the charger. "Change square and analyse again": with a smaller square (0.45, 0.4, side 0.5) the analysis runs again from the screen the button opens, and back on the tab the new square, the 5 new visits and the new visit strip show without leaving the summary.
- **What the check showed about the model (not a player bug; judged by eye on 12 frames of the original clip with the 0.7 square, not a full count):** the clip films a laptop screen playing a YouTube Short that joins several bumblebee videos (a bee on a sunflower, bees on white lantana, a bee flying at yellow flowers in front of a door); the screen's stripes, the laptop logo and the site's buttons are in the frame. On the sunflower scenes the model boxes a brown leaf patch at the lower left of the square (0.72 to 0.75) and a spot next to it (about 0.44), not the bee, which is why Find visits reports pairs of overlapping visits (#1/#2, #10/#11, #16/#17, #21/#22). On the lantana scenes it mostly finds nothing; at 22.5 s it boxes the bee's lower body (0.59) and a flower centre (0.40). The flying bee had no box in the frames looked at. So it finds the bee rarely and only in part, and flags leaf and flower spots. The run with the smaller square was not checked frame by frame. This is the kind of problem the Video tab is meant to expose.
- **Tests:** `video_box_timeline_test.dart` (8: hold rule with gaps and tail, a first frame at 50.3 ms, class names and clip separation, the square from `roi_px` / settings / whole frame, tracked boxes from a real `VideoTracker.run` land on the detector's boxes with a 1080-px square, stale visits after a new run, torn line and missing file, a clip analysed in part is not tracked); `video_review_player_test.dart` (3, fake `VideoPlayerPlatform`, 360 px with a bottom system bar: the full flow of speed, seek, visit jumps, both switches, legend, visit row, last button above the inset, pause on tab change, dispose; no square and no visits hides both switches and disables the visit buttons, the second clip's "not analysed" note; a live session keeps Photos and never opens a player).
- **Verification:** `flutter analyze` clean; `flutter test test/fauna_pulse` 643 pass (2 skipped, as before).

## Round 232 (2026-09-25): video plan Phase 2b, the phone's temperature, power and speed while the AI runs on videos

The summary's extra graphs stayed empty for imported video sessions: nothing measured the phone during "Run AI on videos". Researchers also had no file for the phone's state during an analysis.

- **Samples (`postprocess/video_detector.dart`):** every run now writes, next to the boxes in `video_detections.jsonl`, the live records `thermal` (`DeviceThermal.toJson()` + `clip`, + `paused: true` while waiting to cool down) and `power` (the live `_samplePower` fields), plus a new `analysis_speed` record for the time since the previous sample (`clip`, `period_ms`, `frames`, `frames_decoded`, `frames_per_s` on the clock, per analysed frame `decode_ms` / `convert_ms` / `detect_ms`, `paused_ms`; the means are left out with no frame, the record with no frame and no pause). The three records of one sample share `time_ms`. The first sample is at the first reading, then on a grid every "Measure the phone every" seconds, also while paused (the pause poll becomes min(15 s, interval)), and one last sample at the end of a run with work. `video_thermal_pause` (`temp_c`, `limit_c`, `resume_below_c`) and `video_thermal_resume` (`paused_ms`, `cancelled`) mark each cooling pause; `video_run_start` gets `sample_s`. `detect_ms` is the native `inferMs`, which times the whole `predict()` call (input preparation, model, box decoding); the help text says so.
- **`logging/thermal_pause.dart`:** `waitWhileWarm` calls `onReading(reading, paused)` for every reading and returns it in `ThermalWait.reading`, so the sampler never reads the sensors a second time; `sleep` is injectable (tests run on a fake clock). Identification's use is unchanged.
- **New setting "Measure the phone every"** (Run AI on videos → Advanced settings, 10 s, 5 to 60, pref `video_analysis_sample_s`; Setup-tab row "Measure the phone every", SETTINGS_REFERENCE row). 10 s is the live default for temperature and power.
- **`postprocess/video_run_samples.dart`** (new, parsed in an isolate): puts the runs back to back on one analysis clock, with a gap of max(60 s, 5 × the longest interval) between runs so the graphs break their lines there; a run without samples takes no room; a killed run ends at its last `raw_detections` (read by prefix, not decoded), and a pause still open ends there too. It counts frames from the `raw_detections` lines.
- **Summary, imported sessions (Graphs → Extra graphs):** "While the AI ran on the videos" replaces the live graphs, with a numbers row (analysis time and runs, frames, speed while running = frames / (run time − pauses), cooling pauses, energy on battery or "not measured: the phone was plugged in"), then battery temperature, headroom (or the "not reported" note), frames analysed per second, detector ms per frame and power (hidden when charging or plugged, as live). Cooling pauses are grey bands (`_SeriesPainter.shade`); all graphs share the analysis clock (`_series(range:)`). The Wh integral and the power smoothing skip steps as long as a run gap (`_buildEnergySeries(maxStepMs:)`), so the hours between two runs are never counted. Re-opening the analysis resets the old samples and energy figures.
- **`phone_during_analysis.csv` in Share results**, one row per sample: `run, time_s, epoch_ms, clip, temp_c, headroom, thermal_status, power_w, battery_current_ua, battery_voltage_mv, charging, plugged, frames_per_s, frames, decode_ms, convert_ms, detect_ms, paused, paused_ms`. Beyond the plan's columns: `run` and `epoch_ms` (to split runs and join the JSONL), the two raw battery columns (to apply the DATA_GUIDE §3 corrections; `power_w` stays raw, and the Xiaomi's two-cell voltage doubles it), `frames` (to weight per-frame means) and `paused_ms`. DATA_GUIDE §9 documents the records, the graphs and the CSV, with an R and a pandas example that weight `detect_ms` by `frames`.
- **Found in the device check and fixed:** the stats line under "Frames analyzed per second" averaged all points, so the eight 0-fps samples of a 45 s pause gave "Average 12.7 fps (median 0.0)" under "Speed while running 39.3". It now reads "Without the cooling pauses: median … fps; min …, max …. The average is "Speed while running" above." (periods with `paused_ms` left out, `VideoRunSamples.framesPerSRunning`). The same check showed why the CSV needs `frames`: run 1's first period (105 frames, 31.3 ms, the GPU warming up) and its short last one (48 frames, 8.3 ms) average 19.8 ms per row but 24.3 ms per frame.
- **Device check (Xiaomi 2b2dc560, debug build, new `integration_test/video_samples_check_test.dart`, `VID_20260924_155954.mp4` 47.8 MB, 720×1280, 30.4 s, every frame (30 per second), whole frame, `arthropod_yolov11_float16.tflite` on the GPU, phone on the charger at 100 %, 30 to 31 °C):** run 1 stopped after 6 s (161 frames), run 2 with the pause limit 1 °C under the battery's temperature paused from its first reading and was stopped after 45 s, run 3 finished the clip (747 frames in 16 s). Samples came every 5.01 s in the pause and on the 5 / 10 s grid while running; run 3's frame-weighted `detect_ms` 8.50 vs `video_clip_done` 8.54 ms per frame (decode 1.16 vs 1.13, convert 11.04 vs 11.00; the rows are rounded to 0.1 ms). Summary: "1m 8s in 3 runs", 908 frames, 39.8 frames per second, "1 (0m 45s in total)", energy not measured (plugged), grey band and run gaps as intended, headroom not reported by this phone, power graph hidden. The zip holds the CSV (16 rows); the R and pandas examples of DATA_GUIDE §9 run on the pulled file (per-frame detector ms run 1 23.2, run 3 8.5). Debug build, a single short clip, on the charger: not performance figures. Two things to know from it: with the phone at 100 % on the charger the battery current reads exactly 0 while idle, which the native reader reports as missing, so `power_w` is empty in most pause rows (as in live sessions); and on the whole 720×1280 frame cutting out and scaling the picture (`convert_ms` about 11 ms) took longer than the detector on the GPU (about 8.5 ms), a lead for later speed work.
- **Tests:** `video_detector_test.dart` +2 (fake clock and thermal: cadence 3 s with thermal at 0/3/6/9/11 s and the speed figures per period; a pause from 1 to 11 s with samples inside it, `paused_ms` and the resume record); new `video_run_samples_test.dart` (4: two runs and the gap, killed while paused, no file or no samples, the CSV from two real fake-clock runs against the golden file `fixtures/phone_during_analysis.csv`, including a clip name that needs quoting, and the same CSV inside the Share results zip); `video_screens_test.dart` (prefs round-trip with the new setting and its default, the Setup row, and a 360-px summary test of the numbers row, the graph titles, the running-fps line, Wh without the run gap and the last row above the bottom inset).
- **Verification:** `flutter analyze` clean; `flutter test test/fauna_pulse` 650 pass (2 skipped, as before).

## Round 233 (2026-09-25): faster video frame conversion, and a tip when the video picker hides a file

The owner asked whether the convert step of "Run AI on videos" (about 11 ms per frame on the whole 720×1280 frame, as long as detection, debug build) was worth speeding up, and reported that the importer did not list `Download/ForFaunaPulse/TestVideos/VID-20260924-WA0005.mp4`.

- **Convert on several cores (`VideoFrameSource.kt`):** the YUV→RGB loop moved into `convertRows(FrameJob, from, to, RowBuffers)`, with the same per-pixel integer math. A small pool of half the cores (at most 4, daemon threads "video-convert", shut down in `close()`) and the calling thread take chunks of rows (at least 8, about outH / (workers × 8)) from an `AtomicInteger` until none are left. Chunks rather than equal shares because the phone mixes fast and slow cores; equal bands gave 5.2 to 5.7 ms, chunks 4.4 to 5.2 ms. Each band reads through its own `ByteBuffer.duplicate()` and row buffers. `convert()` waits for every band before returning, because the caller closes the `Image` right after, and the first failure is re-thrown. The bitmap is now filled with `copyPixelsFromBuffer(IntBuffer.wrap(pixels))`, with the pixels written in the bitmap's own RGBA byte order (`A<<24 | B<<16 | G<<8 | R` as a little-endian int), which saves `setPixels`' per-pixel conversion. A temporary timer run (removed) put the remaining ~3.7 ms at: row conversion 3.0, bitmap copy 0.6, `releaseOutputBuffer` 0.6, `getOutputImage` 0.1. `convert_ms` is now the waiting time, not the summed work of the cores (DATA_GUIDE §9).
- **Picture size left at 1280 (`VideoRunConfig.maxSidePx`):** shrinking straight to the model's input size (320 px for `arthropod_yolov11_float16.tflite`) was also tried. It was about twice as fast (66.6 vs 36.5 frames per second, convert 3.7 vs 11.8 ms, detect 7.4 vs 11.2 ms) but changed half the boxes (391 → 206; 203 matched at mean IoU 0.92; most unmatched ones below 0.4) and Find visits went from 3 to 0. The model then sees an averaged picture instead of its own shrink, which live AI uses too. Whether that helps or hurts needs a hand-counted field clip. The owner's test clips film a laptop screen playing a bumblebee video (artefacts the detector was not trained on, which field use will not have), so they serve for checking features and exact equality, not for judging detection. The size stays 1280 (no change to the settings identity, so earlier runs continue without a prompt); the doc comment says why.
- **Video picker tip (`home_screen.dart`):** the system picker's Downloads view lists only MediaStore rows with `is_download=1`. The WhatsApp copy has `owner_package_name=com.whatsapp, is_download=0` (the other clip in the same folder: owner NULL, `is_download=1`), so it is missing there but listed under ☰ → the phone's name → Download → ForFaunaPulse → TestVideos (checked on the Xiaomi) and under Videos. FaunaPulse filters nothing: file_picker asks for `video/*`, and both rows are `video/mp4`. When the picker closes without a choice (and without an error), a snack bar now says: "Video not in the list? In the file window, tap ☰ (top left) and choose Videos, or your phone's name and then the same folder. The Downloads view hides some files, for example videos saved by WhatsApp." It has a "Try again" action and disappears after 12 s (`persist: false`; a snack bar with an action would otherwise stay). It is built by the public `videoPickTipSnackBar(onRetry:)`. `docs/VIDEO_ANALYSIS.md` §1 got the same explanation.
- **Device check (Xiaomi 2b2dc560, debug build, new `integration_test/video_convert_check_test.dart`, 15 frames per second, `arthropod_yolov11_float16.tflite` on GPU, phone on the charger at 100 %, 29 to 31 °C):** the check runs a clip twice (the two runs must agree exactly) and compares every frame's boxes with a reference saved by an earlier run (per clip, model, size and square in `video_convert_check_ref/`). The references were made with the old converter (the file from HEAD put back for the runs, then the new one restored). With the new converter, all four cases matched the reference on 455 of 455 frames, box for box and digit for digit:
  - `VID_20260924_155954.mp4` whole frame at 1280 (391 boxes): convert 10.8 to 12.5 → 4.4 to 5.2 ms per frame; one run 39.4 → 57.7 to 58.3 frames per second;
  - the same clip with the square 0.5, 0.6, 0.7 (431 boxes): 3.2 to 3.3 → 2.7 ms;
  - the same clip shrunk to 400 px, a whole-number step of 3 (170 boxes): 4.4 → 2.9 ms;
  - `VID-20260924-WA0005.mp4`, 478×850 (213 boxes): 3.9 to 4.0 → 3.4 ms.
  The second run of each of the last three was slower in every step, decoding included (the phone had just been reconnected), so only the first runs count for speed. The first run after loading the model has slower detection (13 to 15 ms vs 8.5 to 10 ms) in every build. The tip was checked on the phone after reinstalling the normal debug app: home ⋮ → Import videos… → Back → the snack bar sits above the navigation bar, with "Try again" visible.
- **Tests:** new `home_video_pick_tip_test.dart` (360 px: the text names Videos and WhatsApp, Try again calls back, and the bar is gone after 13 s).
- **Verification:** `flutter analyze` clean; `flutter test test/fauna_pulse` 651 pass (2 skipped, as before). In a first full run `session_log_index_test.dart` "100k-line session parses completely" timed out at its 30 s limit; alone it passes in about 24 s, and the second full run passed, so it is a slow test near its limit under parallel load, not this round's code (untouched).

## Round 234 (2026-09-26): video plan Phase 2c, frames kept of each visit, shown under the player and used by identification

The owner asked for Phase 2c of the video plan on top of Round 233: keep pictures of each visit found in the videos, by the live photo rule, show them on the Video tab, and let identification run on them.

- **One photo rule for live and video (`capture/roi_capture.dart`):** the per-track window logic of `RoiCaptureScheduler.evaluate` moved unchanged into the pure `TrackKeepRule.due(ids, nowMs)` (first frame at once, then one every step, until the duration after the track was first seen; a window is forgotten only after the track has been gone longer than the duration). The live scheduler calls it exactly where it ran the loop before (still skipped while a photo is being saved), so live photos do not change; the existing scheduler tests pass untouched.
- **Choosing frames (`postprocess/video_tracker.dart`):** `VideoTracker.run(..., keep: KeepFramesSettings?)` evaluates the rule on every tracked frame (so ended visits are forgotten as live) and, for a due frame, writes a live-shaped `capture` record (`file` = `roiPhotoFileName(frame ms, file_token)` with the session's token from `session.jsonl`, `video` when there is none; `captured_at_ms`, `track_ids`, `source: video`, `clip`, `frame`, `pts_us`, `roi_px`, `saved_px` for a square or `saved_w`/`saved_h`), and `jpeg` on the track entries it was kept for. Visits in the same frame share one picture. `post_track_start` gets `keep_frames` (`{step_seconds, duration_seconds}` or null) and `file_token`; `post_track_end` gets `kept_frames`; `PostTrackSummary` exposes `runId`, `keep`, `keptFrames`. `readKeptFrames(dir)` / `KeptFrame` read the list back. Files are protected: a name already taken moves on by 1 ms unless the previous run kept that name and its clip is still in `videos/` (or it is the same picture); after the new `post_tracks.jsonl` is complete, frames the old run kept and this one does not keep as the same picture are deleted, except those whose video is gone (they could not be made again). A crash in between leaves old records pointing at missing files, which every reader skips, never a name pointing at the wrong picture.
- **Saving them (`VideoFrameSource.kt`, `YOLOPlugin.kt`, plugin `video_frame_source.dart`, new `postprocess/video_frame_keeper.dart`):** new channel methods `videoOpenFrames(path, roiPx)` and `videoSaveFrames(ptsUs[], paths[], quality 90, budgetMs 1500)` on the existing single video thread and source slot. The decoder goes forward through the clip, seeks to the key frame before the next wanted frame when it is more than 3 s ahead, converts the analysed area at full size with the Round 233 converter, and writes each JPEG under a `.tmp` name then renames it. A frame missing from the video is replaced by the next one; frames past the clip's end come back as `missing`; a decoder silent for 5 s is an error. `VideoFrameKeeper.run` takes the kept frames whose file is not there yet, groups them per clip and area in `pts_us` order, asks for 12 per call (progress and Stop stay responsive), and counts the frames of a missing or unreadable clip as failed (a call that makes no progress is an error, not a loop). `VideoFrameKeeper.status` gives saved / total / no video for the screen.
- **"Run AI on videos" screen (`video_analysis_screen.dart`):** Visits section: switch *Keep frames of each visit* (on), *Keep a frame every* (1 s, 0.1 to 10) and *For up to* (10 s, 1 to 300; the helper says how many frames a visit gives at most), prefs `video_analysis_keep_frames` / `_keep_step_s` / `_keep_duration_s`; a changed rule counts as changed settings for *Find visits again*. With the switch off and frames saved, an amber note says a new *Find visits* removes them. After *Find visits* the frames are saved at once (progress bar, "Saving the kept frames: X of Y", Stop); afterwards "Kept frames saved: X of Y", a note for frames whose video is gone, and *Save the remaining N frames*. Analysing, finding visits and saving frames exclude each other (`_busy`), since they share the one native video source. The wakelock calls became best effort (`_keepAwake`): a refused wakelock no longer stops a run or leaves its flag set. The decoder can be replaced for tests through the constructor (`frameSaveBackend`).
- **Video tab (`session_summary_screen.dart`, `video_review_player.dart`, `session_log_index.dart`):** under the player a *Kept frames* section with the existing photo viewer (random sample of 10, pick more, boxes, info panel), counting only frames saved so far. `IndexedPhoto` carries `clip` and `ptsUs`, and the index reads `saved_w`/`saved_h`. Frames wider than tall (whole picture analysed) are letterboxed in the viewer; boxes and the crop tool map into the picture's rect. The tool row gets *Show in video*, which calls `VideoReviewPlayerState.showMoment(clip, pts_us ÷ 1000)`: pause, open the clip if needed, seek, scroll back up to the player. The info panel gets "In video: <clip> at m:ss.s". The player's list stops scrolling while the viewer is zoomed. The Setup tab of an imported session shows "Kept frames per visit". The photos reload after returning from *Run AI on videos*.
- **Identification (`identification_job.dart`, `identification_store.dart`, `identification_results_screen.dart`):** kept frames are photos in `roi_frames/` with `capture` records and `jpeg` entries, so planning, cropping and the gallery copy needed no change. Because *Find visits* numbers the visits anew each run, `identify_start` stores `visits_run_id` (the current `post_track_start.run_id`, sessions whose visits were found afterwards only), `EmbeddingIndex.visitsRunId` reads it back, and a run whose stored id differs deletes the embeddings and starts over (logged as `identify_resume_reset`). Scoring reads `post_track_start` too: the summary's `capture` gets `visits_run_id`, and `photo_step_s`/`photo_duration_s` come from the kept-frames rule. The results screen compares the stored id with `VideoTracker.readSummary().runId` and, when they differ, shows the amber line "The visits were found again since; run identification again." with a tap-for-why explanation.
- **Tests:** new `video_kept_frames_test.dart` (10: frames = visits × (1 + M/N) with records and summary fields; wide frames; off; a foreign file is never overwritten; stale frames deleted but not without their video; keeper saves once, resumes after Stop, fails cleanly on a missing video or a stalled decoder; index reads clip and moment; identification end to end: same visits continue, found again starts over under the new run id). `video_screens_test.dart`: Find visits saves the kept frames through a fake decoder, the new rows appear and the off-note shows (360 px); the prefs round trip covers the three settings. `video_review_player_test.dart`: kept frames under the player, *Show in video* seeks the player to the frame and brings it back into view; the old case scrolls past the new section. `identification_results_screen_test.dart`: warning shown only for a different run id.
- **Docs:** DATA_GUIDE §9 (new "Kept frames: `roi_frames/`" part, `post_tracks.jsonl` fields, `visits_run_id` in `identify_start` and `summary_<pack>.json`), VIDEO_ANALYSIS §1, SETTINGS_REFERENCE (three rows), overview row.
- **Device check (Xiaomi 2b2dc560, debug build, new `integration_test/video_keep_frames_check_test.dart`, both VID test clips (478×850 and a 720×1280 clip stored rotated 90°), 15 frames per second, whole picture, `arthropod_yolov11_float16.tflite` on GPU, phone on the charger, 32 °C):** analysis 910 frames in 20 s; *Find visits* 5 visits, 21 frames kept (at most 5 × 11 = 55), every visit with a frame, 0.24 s. Saving: 21 of 21 in 1.6 s (76 to 79 ms per frame, about 122 KB each), 0 missing, 0 failed, each JPEG the size of the analysed area (upright). Four frames saved again straight from the decoder came back at exactly the wanted `pts_us` with byte-identical files. Stopped half way: 19 saved, the next run saved the other 2, same bytes. *Find visits* again every 2 s: 13 frames kept, the 8 unchanged files untouched (same modification time), the 13 no longer kept deleted, 5 new ones saved in 0.8 s. Identification (`bioclip-2_image_fp16.tflite`, CPU after the GPU request, first pack found): 18 crops from the 13 frames in 56 s, 5 track ids, summary `capture` = `{photo_step_s: 2.0, photo_duration_s: 10.0, visits_run_id: <the run>}`; after *Find visits* again (every 1 s) the next run continued 0 crops and embedded 30 under the new run id. (The clips film a laptop screen, so the taxa say nothing about accuracy.) Video tab: "Showing 10 of 21 kept frames"; the 478×850 frame letterboxed with its box on the bee and "In video: VID-20260924-WA0005.mp4 at 0:12.2"; *Show in video* paused the player at exactly that moment (12206 and 10541 ms in two runs) and scrolled back up to it (screenshots). The first run failed only on the check's own hit test on `VideoPlayer` (the box overlay covers it, as noted in the round 231 check); it now looks for the Play button, and the second run passed. Seen on the screenshot, not changed: with a tall portrait clip at the top of the tab, the player's control row sits behind the navigation bar until the list is scrolled a little (round 231 layout).
- **Verification:** `flutter analyze` clean; `flutter test test/fauna_pulse` 664 pass (2 skipped, as before); debug build compiles the Kotlin changes.
