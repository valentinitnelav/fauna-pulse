...
class _YOLOViewState extends State<YOLOView> {
  ...
  void didUpdateWidget(YOLOView oldWidget) {
    ...
    // Re-apply streaming config when it changes (e.g. analysis resolution or
    // inference rate edited in settings). The native side rebinds the camera
    // only if the analysis resolution actually differs, so this is cheap when
    // nothing material changed.
    final newCfg = widget.streamingConfig;
    if (newCfg != null &&
        newCfg.toString() != oldWidget.streamingConfig?.toString()) {
      _effectiveController.setStreamingConfig(newCfg);
    }
  }
