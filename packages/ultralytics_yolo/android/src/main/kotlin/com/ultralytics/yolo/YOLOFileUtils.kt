...
object YOLOFileUtils {
    ...
    /**
     * The (height, width) of a TFLite model's first input tensor, read straight
     * from the model graph. The input tensor shape is always present in a TFLite
     * file (unlike the optional Ultralytics "imgsz" metadata), so this lets the UI
     * show an input resolution for ANY .tflite — including non-YOLO exports
    ...
    fun inputImageSize(context: Context, modelPath: String): Pair<Int, Int>? {
        ...
        return try {
            val shape = org.tensorflow.lite.support.metadata.MetadataExtractor(buffer)
                .getInputTensorShape(0)
            if (shape == null || shape.size != 4) return null
            // Batch is shape[0]. A channel dim of 1 or 3 identifies the layout:
            // NCHW = [1, C, H, W]; otherwise NHWC = [1, H, W, C].
            if (shape[1] == 3 || shape[1] == 1) {
                Pair(shape[2], shape[3]) // NCHW -> (H, W)
            } else {
                Pair(shape[1], shape[2]) // NHWC -> (H, W)
            }
        } catch (e: Exception) {
            Log.w(TAG, "Input tensor shape read failed for $modelPath: ${e.message}")
            null
        }
    }

    /**
     * Full input tensor shape (e.g. [1, 320, 320, 3]) read from the TFLite graph
     * by index — no dependency on the input tensor being named "images". Used as a
     * fallback so models exported with a differently-named input (older/renamed
     * YOLO exports, non-Ultralytics exports) still get the right preprocessing size
     * instead of the hard-coded 640 default. [modelPath] is an absolute file path
     * (the resolved path the runtime actually loads). Null on any error.
     */
    fun inputTensorShapeFromPath(modelPath: String): IntArray? = try {
        val f = File(modelPath)
        if (!f.isAbsolute || !f.exists()) null
        else FileInputStream(f).channel.use { ch ->
            val buf = ch.map(FileChannel.MapMode.READ_ONLY, 0, ch.size())
            org.tensorflow.lite.support.metadata.MetadataExtractor(buf).getInputTensorShape(0)
        }
    } catch (e: Exception) {
        Log.w(TAG, "inputTensorShapeFromPath failed for $modelPath: ${e.message}")
        null
    }

    /**
     * Maps a model file to a ByteBuffer, resolving the path the way the model is
     * actually stored. Imported models are absolute files; Flutter-bundled models
     * live under "flutter_assets/<declared path>" inside the APK's assets (NOT at
     * the bare declared path), so that prefix is tried too. Returns null if none
     * of the candidates can be opened. (The plugin's own [YOLOUtils.loadModelFile]
     * doesn't try the flutter_assets prefix, which is why asset models read back
     * as "unknown" before this.)
     */
    private fun openModelBuffer(context: Context, modelPath: String): ByteBuffer? {
        val withExt = YOLOUtils.ensureTFLiteExtension(modelPath)
        // Absolute imported file first.
        for (p in listOf(withExt, modelPath).distinct()) {
            val f = File(p)
            if (f.isAbsolute && f.exists()) {
                try {
                    ...
                    Log.w(TAG, "openModelBuffer: file map failed for $p: ${e.message}")
                }
            }
        }
        // Then assets: Flutter bundles under "flutter_assets/", but also try the
        // raw key in case the model is a plain native asset.
        val assetKeys = listOf(
            "flutter_assets/$withExt",
        ...
        for (key in assetKeys) {
            try {
                context.assets.openFd(key).use { afd ->
                ...
                // Asset key not present — try the next candidate.
            }
        }
        return null
    }
