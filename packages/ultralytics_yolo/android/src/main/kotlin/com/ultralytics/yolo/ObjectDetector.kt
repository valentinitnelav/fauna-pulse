...
class ObjectDetector(
    ...
    override fun predict(bitmap: Bitmap, origWidth: Int, origHeight: Int, rotateForCamera: Boolean, isLandscape: Boolean): YOLOResult {
        ...
        // When a region of interest is set we crop to it before inference, and
        ...
        val effHeight: Int
        if (roi != null) {
            val roiPx = ImageUtils.prepareBitmapForModelRoi(
                bitmap = bitmap,
        ...
        for (boxArray in resultBoxes) {
            if (boxArray.size >= 6) {
                ...
                if (rect != null) {
                    val normRect = normalizedRectFromInputRect(rect, effWidth, effHeight)
        ...
        return YOLOResult(
            origShape = com.ultralytics.yolo.Size(effWidth, effHeight),
            boxes = boxes,
